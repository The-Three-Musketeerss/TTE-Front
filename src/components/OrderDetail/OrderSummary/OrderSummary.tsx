import { orderProps } from "@utils/types";

const OrderSummary = (order: orderProps) => {
  return (
    <div className="w-full max-w-lg 2xl:max-w-lg bg-white shadow-md rounded-md p-4 flex flex-col gap-4">
      <h3 className="font-semibold text-xl">Order Summary</h3>
      <div className="flex justify-between items-center mt-4">
        <span className="text-sm font-semibold">Address</span>
        <span className="text-sm font-semibold">{order.address}</span>
      </div>
        <div className="flex justify-between items-center mt-2">
            <span className="text-sm font-semibold">Payment Status</span>
            <span className="text-sm font-semibold">{order.paymentStatus}</span>
        </div>
        <div className="flex justify-between items-center mt-2">
            <span className="text-sm font-semibold">Order Status</span>
            <span className="text-sm font-semibold">{order.status}</span>
        </div>
        <div className="flex justify-between items-center mt-2">
            <span className="text-sm font-semibold">Order Date</span>
            <span className="text-sm font-semibold">{new Date(order.createdAt).toLocaleDateString()}</span>
        </div>
      <div className="flex justify-between items-center mt-4">
        <span className="text-sm font-semibold">Subtotal</span>
        <span className="text-sm font-semibold">${order.totalBeforeDiscount}</span>
      </div>
      <div className="flex justify-between items-center mt-2">
        <span className="text-sm font-semibold">Discount</span>
        <span className="text-sm font-semibold">
          ${(order.totalAfterDiscount-order.totalBeforeDiscount).toFixed(2)}
        </span>
      </div>
      <div className="flex justify-between items-center mt-2">
        <span className="text-sm font-semibold">Shipping</span>
        <span className="text-sm font-semibold">${order.shippingCost}</span>
      </div>
      <span className="w-full outline-1 outline-gray-300 flex"></span>
      <div className="flex justify-between items-center mt-2">
        <span className="text-sm font-semibold">Total</span>
        <span className="text-sm font-semibold">${order.finalTotal}</span>
      </div>
    </div>
  );
};

export default OrderSummary;
